...
const attendanceReport = async (req, res) => {
  try {
    ...
    const groupData = attendanceData.reduce((result, record) => {
      const recordDate = record.date?.toISOString?.().split("T")[0] || "Unknown Date";
      ...
      result[recordDate].push({
        employeeId: record.employeeId?.employeeId || "Unknown ID",
        employeeName: record.employeeId?.userId?.name || "Unnamed",
